...
import { catchError, concatMap, retry, map,retryWhen, delay } from 'rxjs/operators';
            ...
            if (count <= retryCount) {
              switch (checkErr.status) {
                case ErrorCode.serverDown:
                  // Simulate a delay or simply retry based on error status
                  return of(checkErr).pipe(delay(1000));  // Example to retry after a delay
              }
            }
